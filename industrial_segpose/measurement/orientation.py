...
import cv2
import numpy as np
from .coordinates import output_axis_angle_from_cv
...
def min_area_rect_orientation(contour: np.ndarray) -> tuple[float, float]:
    ...
    return output_axis_angle_from_cv(long_angle), confidence
...
def pca_orientation(contour: np.ndarray) -> tuple[float, float]:
    ...
    return output_axis_angle_from_cv(float(angle)), float(np.clip(confidence, 0.0, 1.0))
